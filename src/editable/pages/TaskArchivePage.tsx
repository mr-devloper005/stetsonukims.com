...
import { getEditableExcerpt, getEditablePostImage, getEditableCategory, getEditableRawPostImage, getEditableInitials } from '@/editable/cards/PostCards'
...
function ArchiveCard({ task, post, basePath, index }: { task: TaskKey; post: SitePost; basePath: string; index: number }) {
  ...
  if (task === 'profile') {
    const profileImage = getEditableRawPostImage(post)
    return (
      <Link href={href} className="block break-inside-avoid rounded-2xl border border-white/8 bg-[#161616] p-6 text-center transition hover:border-white/15">
        {profileImage ? (
          ...
          <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-[#6366f1]/15 ring-2 ring-white/10">
            <span className="text-xl font-bold text-[#6366f1]">{getEditableInitials(post)}</span>
          </div>
        )}
